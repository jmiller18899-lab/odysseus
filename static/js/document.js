...
import * as Modals from './modalManager.js';
  ...
  /**
   * Make common Babel Standalone snippets work in document HTML previews.
   *
   * LLM-generated React demos often load `@babel/standalone` and then use
   * `<script type="text/babel">` without declaring presets. Babel's browser
   * transformer then tries to parse TS/TSX-looking code without the TypeScript
   * parser enabled, producing the opaque stack seen from `babel.min.js`. Add
   * conservative defaults only to Babel/JSX script tags that are missing them.
   */
  function _withBabelStandaloneDefaults(html) {
    return String(html || '').replace(/<script\b[^>]*\btype\s*=\s*(["'])(?:text\/babel|text\/jsx)\1[^>]*>/gi, (tag) => {
      let next = tag;
      if (/\bdata-presets\s*=/i.test(next)) {
        next = next.replace(/\bdata-presets\s*=\s*(["'])([^"']*)\1/i, (m, quote, value) => {
          const presets = value.split(',').map((p) => p.trim()).filter(Boolean);
  ...
  function toggleHtmlPreview() {
    ...
    if (!_htmlPreviewActive) {
      ...
      const langSelect = document.getElementById('doc-language-select');
      const lang = (langSelect ? langSelect.value : '').toLowerCase();
      iframe.srcdoc = lang === 'html' ? _withHtmlPreviewErrorReporter(_withBabelStandaloneDefaults(code)) : code;
